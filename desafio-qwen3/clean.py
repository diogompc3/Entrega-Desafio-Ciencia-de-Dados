...
def cleaning(txt):
    ...
    fax_date = r'^.*?\d{2}/\d{2}/\d{4}[^\n]*'                               #detectar a primeira quebra de linha caso exista uma data no formato **/**/****.                                      
...
df.dropna(ignore_index = True, inplace = True)              #removendo a linha q retornou NaT
df['id'] = df.index + 1                                     #Corrigindo o index para manter a mesma lógica anterior das noticias brutas
