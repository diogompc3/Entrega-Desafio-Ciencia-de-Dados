...
try:  #checando se os arquivos necessários foram gerados. Deveriam ter sido caso tenham seguido as etapas, então pedindo para executar os scripts anteriores se preciso.
    ...
    print('embeddings.py está corrompido ou se encontra em um formato inesperado.\nPor favor tente executar embed_e5.py novamente.')
    exit()
except FileNotFoundError:
    print('Um dos arquivos requeridos não existem.\nPor favor tente executar clean.py e/ou embed_e5.py.')
    exit()
...
embd_query = model.encode(queries, normalize_embeddings=True)
results = util.semantic_search(embd_query, embd_news, top_k=5, score_function=model.similarity)  #Aqui foram limitados a 3 resultados
print('Os resultados de sua busca são: \n')    
with open(p_respostas / 'output_e5.txt', 'w', encoding='utf-8')as f: #Usado para criar um arquivo com as perguntas/respostas dadas
    for query_id, query in enumerate(queries):
        print(f"Pergunta: {queries[query_id].replace("query: ", "")}", file = f)
        print(f"Pergunta: {queries[query_id].replace("query: ", "")}")
