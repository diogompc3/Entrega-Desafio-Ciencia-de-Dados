...
A pasta contendo os scripts e dados é a pasta [desafio-e5](desafio-e5). Certifique-se da existência do arquivo noticias_brutas.json dentro da pasta dados. 
...
Se desejar executar buscas diferentes das presentes já no código, altere a variável *queries* em [query_e5.py](desafio-e5/query_e5.py) com suas queries, ou use o script query_e5_inputquery.py, que irá te pedir para inserir uma query ao executa-lo para comparar com os vetores. Se for alterar a variável, lembre de adicionar "query: " no início de sua query.

Se desejar visualizar respostas da ultima execução, no caso das vindas do script query_e5.py, estarão presentes em [output_e5.txt](desafio-e5/respostas/output_e5.txt). As respostas das queries vindas de query_e5_inputquery.py estarão em [output_e5_inputquery.txt](desafio-e5/respostas/output_e5_inputquery.txt), e serão gravadas cumulativamente. 
...
Resultados também disponíveis em [output_e5.txt](desafio-e5/respostas/output_e5.txt). Também inclusos os resultados do script querry_qwen.py em [output_qwen.txt](desafio-qwen3/respostas/output_qwen.txt).
...
Nesse caso o modelo teve uma ótima perfomance. Conseguiu buscar as notícias de maior relevância, inclusive encontrando 4 das mesmas respostas do modelo [qwen3-0.6B](desafio-qwen3/respostas/output_qwen.txt), a unica que os modelos discordam sendo uma que ambas dão uma score relativamente baixa.
...
Aqui é vista grande eficiência por parte do modelo e5 novamente. Ele busca corretamente as notícias cujo conteúdo é relevante para a query. Assim ele justifica a escolha por esse modelo em comparação a modelos mais pesados, pelo menos para o caso desse desafio com números limitados de notícias. Ele com sucesso processa as mesmas respostas de maior score que modelos como o qwen3, apenas apresentando algumas respostas de menor score diferentes.
